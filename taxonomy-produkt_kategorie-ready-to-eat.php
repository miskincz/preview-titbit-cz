<?php 
/**
 * Template: Taxonomy - Ready to Eat
 * 
 * Šablona pro zobrazení kategorie "Ready to Eat" a jejích podkategorií
 * Obsahuje: obrázek kategorie, popisek, navigaci a seznam produktů
 * Struktura je stejná jako u Ovoce a zelenina
 */
  ...
  <div class="mainOcCattegories__main">
    ...
    <div class="mainOcCattegories__main__list">
        ...
        $query = new WP_Query([
          ...
          'posts_per_page' => 15, // Změněno z 36 na 15
          'paged'          => $paged,
        ]);
        ...
        if ($query->have_posts()) {
          ...
          // Zobrazit tlačítko pouze pokud je více než 1 stránka
          if($query->max_num_pages > 1 && $paged < $query->max_num_pages) {
            echo '<div class="articlePage">';
            echo '<button id="load-more" data-max-page="'. $query->max_num_pages .'">Načíst další</button>';
            echo '</div>';
          }
